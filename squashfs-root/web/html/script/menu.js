...
function WebCreatMenu()
{
	...
	$.each(window.top.MainMenu, function(mi, mv) {
	    if (window.top.XHas80211acWifi && (mv.id.match("wifi5") != null)) {
	        mv.show = 7;
		}
...
function WebCreatSubMenu()
{
	...
	$.each(cm.submenu, function(si, sv) {
		//alert(si+","+sv.id);
	    if (window.top.XHas80211acWifi && (sv.id.match("wifi5") != null)) {
	        sv.show = 7;
		}
 		...
 		if ((sv.show & window.top.XCurrentUser) != 0) {
			...
			if (sv.submenu)
			{
				...
				$.each(sv.submenu, function(ssi, ssv) {
            	    ...
            	    if (window.top.XHas80211acWifi && (ssv.id.match("wifi5") != null)) {
            	        ssv.show = 7;
                    }
